...
use repovia_executor::{AsyncTcpListener, AsyncTcpStream, NetExecutor};
use repovia_utils::{channel, GenericError, Receiver, Sender};
use std::error::Error;
...
impl AsyncHttpRequestInternal {
    async fn accept_loop_internal(&self) -> Result<(), Box<dyn Error>> {
        ...
        let async_stream = AsyncTcpStream::new(stream, self.executor.clone())?;
    ...
    async fn http_handle_request(
        ...
        let request = AsyncHttpRequestInternal::http_parse_request(data)
            .ok_or(GenericError::new("Error parsing request"))?;
        let uri = request.uri();
...
impl AsyncHttpRequest {
    pub fn new(listener: AsyncTcpListener, executor: NetExecutor) -> Self {
        ...
        executor
            .clone()
            .spawn(async move { internal.accept_loop().await });
